...
- Treat the current workstation database connection as production.
...
- Only execute `SELECT` SQL against the current database connection.
- Do not execute DML or data-changing SQL against the current database connection, including `INSERT`, `UPDATE`, `DELETE`, `MERGE`, `TRUNCATE`, stored procedures that modify data, or transaction-changing commands.
