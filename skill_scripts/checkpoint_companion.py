...
def _render_data_preview(payload: Mapping[str, Any]) -> str:
    summary = {
        "Status": payload.get("status"),
        "Row Count": payload.get("row_count"),
        "Column Count": payload.get("column_count"),
    ...
    if isinstance(columns, list):
        html.extend(["<h3>Columns</h3>", _render_table([{"column": column} for column in columns])])

    checks = payload.get("acceptance_checks")
    if isinstance(checks, Mapping):
        html.extend(["<h3>Acceptance Checks</h3>", _render_key_values(dict(checks))])
    return "".join(html)
...
def _render_payload_sections(checkpoint: str, payload: dict[str, Any]) -> str:
    ...
    if checkpoint == "excel_confirmation":
        ...
        add("Data Preview", _render_data_preview(payload))
