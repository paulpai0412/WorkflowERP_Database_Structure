...
def test_cli_wait_confirmation_accepts_existing_confirmation_by_default(tmp_path: Path):
    run_root = tmp_path / "runs"
    run_dir = run_root / "run-wait-existing-default"
...
def test_cli_classify_workbook_accepts_current_session_payload(tmp_path: Path):
    run_root = tmp_path / "runs"
    run_dir = run_root / "run-current-session"
    workbook = tmp_path / "requirement.xlsx"
    _write_requirement_workbook(workbook)
    classification_payload = {
        "workbook_path": str(workbook),
        "primary_sheet": "Requirement",
        "resolver_strategy": "current_session_codex_table_first_resolver",
        "llm_status": "used_current_session",
        "llm_provider": "current-session",
        "candidate_tables": [
            {
                "table_id": "COPTC",
                "table_name": "Order header",
                "reason": "Current Codex session selected order header table.",
                "confidence": 0.82,
            }
        ],
        "columns": [
            {
                "excel_column": "A",
                "excel_header": "Order No",
                "classification": "db_source_field",
                "processing_location": "formal_db_sql",
                "source_expression": "COPTC.TC001",
                "lookup_sheet": "",
                "field_metadata": [],
                "lineage_inputs": [],
                "relationship_path": [],
                "schema_candidates": [],
                "confidence": "high",
                "reason": "Provided by authenticated current Codex session.",
                "risks": [],
            }
        ],
    ...
    payload_path.write_text(json.dumps(classification_payload, ensure_ascii=False), encoding="utf-8")
    ...
    assert classified.returncode == 0, classified.stderr
    assert json.loads(classified.stdout)["checkpoint"] == "field_formula_classification"
    written = json.loads((run_dir / "data" / "column-classification.json").read_text(encoding="utf-8"))
